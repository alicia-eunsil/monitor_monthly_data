...
- `NEW HISTORY`
  ...
  - 상세 이벤트는 최신 기준시점 이벤트만 표시
  - 이벤트 분석 차트
- `REPORT`
  - 월별/지역별 브리프 템플릿을 마크다운 리포트로 요약
  - DOCX 다운로드 지원(보고서 편집용)
- `AI INSIGHTS` (환경변수로 표시 제어)
  - 기본은 숨김, `SHOW_AI_FEATURES=true`일 때만 표시
  - 영향요인분해(전국 내 경기도 비중 또는 경기도 내 시군 비중)
  - 영향요인분해(지역별)
  - 규칙 기반 인사이트 + AI 보조 해석
  - 시군 요약 화면에서도 AI 인사이트 표시
...
OpenAI 키(선택):
- `OPENAI_API_KEY` 또는 `openai_api_key`로 설정
...
- `NEW HISTORY > 상세 이벤트`는 최신일 기준 이벤트만 표시한다.
