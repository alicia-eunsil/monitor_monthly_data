...
def _build_ai_contribution_commentary(table: pd.DataFrame, meta: Dict[str, Any], point_label: str, yoy_label: str) -> str:
    ...
    up_cnt = int((table["증감"] > 0).sum())
    down_cnt = int((table["증감"] < 0).sum())
    flat_cnt = int((table["증감"] == 0).sum())
    dominant = table.loc[table["증감"].abs().idxmax()] if not table.empty else None
    top_pos_sum = float(top_pos["증감"].sum()) if not top_pos.empty else np.nan
    pos_total = float(table.loc[table["증감"] > 0, "증감"].sum()) if up_cnt > 0 else np.nan
...
def _build_ai_anomaly_commentary(top_df: pd.DataFrame, point_label: str) -> str:
    ...
    dominant_indicator = (
        top_df["지표"].value_counts(dropna=False).index[0]
        if "지표" in top_df.columns and not top_df.empty
        else ""
    )
    dominant_category = (
        top_df["분류"].value_counts(dropna=False).index[0]
        if "분류" in top_df.columns and not top_df.empty
        else ""
    )
    same_pair_cnt = int(
        (
...
def _render_report_template(
    ...
    occupation_df, _ = _compute_contribution_table(report_df, region=region, dataset_key="occupation", lag=lag)
    top_pos_name = "없음"
    top_neg_name = "없음"
    ...
    if region != "전국":
        _, gy_meta_summary = _compute_gyeonggi_vs_national_contribution(compare_base, region_name=region)
    ...
    def _top_factor_text(table: pd.DataFrame, unit: str, positive: bool) -> str:
        ...
        return f"{_escape_markdown_text(row['분류'])}({_fmt_num(row['증감'], unit)}, {pct_text})"
